...
def farthest_point_sample(pointA, pointB, correspondence, npoint):
    ...
    # Create an array to store corresponding points from pointB
    verts2 = np.zeros((npoint, D), dtype=pointB.dtype)
    new_corres = np.identity(npoint)
    for i in range(npoint):
        if correspondence[centroids[i]].any():
            # If there is at least one correspondence, choose one of them
            verts2[i] = pointB[correspondence[centroids[i]].nonzero()[0][0]]
        else:
            # If there is no correspondence, set to (0, 0, 0)
            verts2[i] = np.array([0, 0, 0], dtype=pointB.dtype)
    return verts1, verts2, new_corres
