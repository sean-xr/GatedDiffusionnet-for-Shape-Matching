...
class DiffusionNet(nn.Module):
    ...
    def __init__(self, C_in, C_out, C_width=128, N_block=4, last_activation=None, outputs_at='vertices',
                 ...
                 with_gradient_features=True, with_gradient_rotations=True, diffusion_method='spectral', gated=False):
        """
    ...
    def forward(self, x_in, mass, basis, L=None, evals=None, evecs=None, gradX=None, gradY=None, edges=None, faces=None):
        """
        ...
        Parameters:
            ...
            basis (tensor):     Axiomatic Basis of shape [B, N, C_out]
        ...
        for b in self.blocks:
            ...
            torch.save(x, f"/content/drive/MyDrive/P3DCV/feature_maps/{i}.pt")

        # Apply the last linear layer
        x = self.last_lin(x)
        gated_basis = self.basis_gate(basis)
        x = x * gated_basis
...
class FunctionalMapCorrespondenceWithDiffusionNetFeatures(nn.Module):
    ...
    def __init__(self, n_feat=128, input_features="xyz", lambda_param=1e-3, gated=False):
        super().__init__()
    ...
    def forward(self, shape1, shape2):
        verts1, faces1, frames1, mass1, L1, evals1, evecs1, gradX1, gradY1, hks1, basis1 = shape1
        verts2, faces2, frames2, mass2, L2, evals2, evecs2, gradX2, gradY2, hks2, basis2 = shape2

        basis1, basis2 = basis1.to(torch.float32), basis2.to(torch.float32)
        ...
        feat1 = self.feature_extractor(features1, mass1, basis1, L=L1, evals=evals1, evecs=evecs1,
                                       gradX=gradX1, gradY=gradY1, faces=faces1)
        feat2 = self.feature_extractor(features2, mass2, basis2, L=L2, evals=evals2, evecs=evecs2,
...
class MLP2(nn.Sequential):
    '''
    A simple MLP with configurable hidden layer sizes.
    layer_sizes: list of ints, like [3, 128, 128]
    '''

    def __init__(self, layer_sizes, dropout=False, activation=nn.ReLU, name="MLP"):
...
class MLP3(nn.Module):
    '''
    A simple MLP with a two-headed output for regressing point cloud data to a (B, N, 100) label.
    layer_sizes: list of ints, like [3, 128, 128]
    '''
    def __init__(self, layer_sizes, dropout=False, activation=nn.ReLU, name="MLP"):
        ...
        self.value_head = nn.Linear(layer_sizes[-2], layer_sizes[-1])
        self.zero_head = nn.Linear(layer_sizes[-2], layer_sizes[-1])

    def forward(self, x):
        x = self.common_layers(x)
        value = self.value_head(x)
        zero_prob = torch.sigmoid(self.zero_head(x))
        return value * (1 - zero_prob)
